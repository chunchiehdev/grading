...
export async function loader({ request }: { request: Request }) {
  try {
    ...
    return Response.json(createErrorResponse('sessionId or resultId is required', ApiErrorCode.VALIDATION_ERROR), {
      status: 400,
    });
  } catch (error) {
...
export async function action({ request }: { request: Request }) {
  try {
    ...
    if (!resultId) {
      return Response.json(createErrorResponse('Result ID is required', ApiErrorCode.VALIDATION_ERROR), {
        status: 400,
      });
    }
    ...
    switch (action) {
      case 'complete': {
        ...
        if (!gradingDataStr) {
          return Response.json(createErrorResponse('Grading data is required', ApiErrorCode.VALIDATION_ERROR), {
            status: 400,
          });
        }
      ...
      case 'fail': {
        ...
        if (!errorMessage) {
          return Response.json(createErrorResponse('Error message is required', ApiErrorCode.VALIDATION_ERROR), {
            status: 400,
          });
        }
