...
describe('Gemini 2.0 Flash Rate Limit Load Tests', () => {
  ...
  describe('Rate Limit Validation', () => {
    it.skipIf(skipReason)(
      'should hit 15 RPM limit and handle gracefully',
      async () => {
        ...
        for (let i = 0; i < gradingResults.length; i++) {
          ...
          try {
            const result = await processGradingResult(gradingResults[i].id, teacher.id, gradingSession.id);
            ...
            if (rateLimitTracker.getRequestsInCurrentMinute() >= 15) {
              rateLimitHit = true;
              console.log(
                `⚠️  Request ${i + 1}: Rate limit reached (${rateLimitTracker.getRequestsInCurrentMinute()}/15)`
              );
            } else {
              console.log(
    ...
    it.skipIf(skipReason)(
      'should recover from rate limiting and continue processing',
      async () => {
  ...
  describe('Concurrent PDF Processing Load', () => {
    it.skipIf(skipReason)(
      'should handle 20+ concurrent student submissions',
      async () => {
  ...
  describe('Database Connection Load', () => {
    it.skipIf(skipReason)(
      'should handle database connections during high concurrency',
      async () => {
