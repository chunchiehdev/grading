...
export const REAL_API_CONFIG: RealApiTestConfig = {
  ...
  loadTesting: {
    ...
    maxCostPerTest: 5.00     // $5 budget limit per test run
  },
...
export class RateLimitTracker {
  ...
  isRateLimited(): boolean {
    return this.getRequestsInCurrentMinute() >= REAL_API_CONFIG.gemini.rpmLimit ||
...
export async function waitForRateLimit(rateLimitTracker: RateLimitTracker): Promise<void> {
  while (rateLimitTracker.isRateLimited()) {
    console.log(`⏳ Rate limited. Waiting... ${rateLimitTracker.getStatus().requestsThisMinute}/15 requests this minute`);
