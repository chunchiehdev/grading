...
export function EmptyGradingState({}: EmptyGradingStateProps) {
  ...
  return (
    <div ref={containerRef} className="flex flex-col items-center justify-center h-full text-center space-y-4 p-8">
      {/* Main Icon */}
      <div ref={mainIconRef} className="text-muted-foreground mb-2">
        <Sparkles className="w-12 h-12" />
      </div>

      {/* Title & Description */}
      <div ref={stepsRef}>
        <h3 className="text-lg font-medium text-foreground mb-2">{t('grading:feedbackUi.empty.title')}</h3>
        <p className="text-muted-foreground">{t('grading:feedbackUi.empty.subtitle')}</p>
      </div>
    </div>
  );
}
