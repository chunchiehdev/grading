...
import { RateLimitTracker, CostTracker, createMinimalTestContent } from './real-api-config';
import {
  UserFactory,
  RubricFactory,
  UploadedFileFactory,
  ...
  describe('PDF Parsing Bottlenecks', () => {
    it('should handle PDF parsing delays and timeouts', async () => {
      ...
      mockProcessGradingResult.mockImplementation(async (gradingResultId: string) => {
        ...
        if (fileSize > 10 * 1024)
          parsingTime = 5000; // 5s for large files (>10KB)
        else if (fileSize > 5 * 1024)
          parsingTime = 2000; // 2s for medium files (>5KB)
    ...
    it('should handle concurrent PDF uploads and parsing', async () => {
      ...
      const averageProcessingTime =
        processingResults.reduce((sum, r) => sum + r.processingTime, 0) / processingResults.length;
    ...
    it('should handle memory and CPU load during intensive PDF processing', async () => {
      ...
      const testFiles = await Promise.all(
        Array.from({ length: 20 }, (_, i) => {
          ...
          if (complexity > 0.8)
            fileSize = 15 * 1024; // 15KB for complex files
