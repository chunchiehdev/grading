...
export function AgentChatBoxWithSteps() {
  ...
  // Navigate to session (used by layout via callback if needed)
...
function MessageBubbleWithSteps({ message, user }: { message: UIMessage; user: UserType | null }) {
  ...
  // For user messages, show simple text without avatar
  if (isUser) {
    return (
      <div className="flex justify-end">
        <div className="text-sm whitespace-pre-wrap break-words px-3 sm:px-4 py-2 rounded-2xl bg-muted dark:bg-muted/60 max-w-[85%]">
          {messageContent}
        </div>
      </div>
    );
