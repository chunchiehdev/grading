...
export class ProtectedAIService {
   ...
   * 受保護的 OpenAI 調用
   */
  static async callOpenAI<T>(
    operation: () => Promise<T>,
    operationName = 'openai-general'
  ): Promise<T> {
   ...
   * 受保護的 Google AI 調用
   */
  static async callGoogleAI<T>(
    operation: () => Promise<T>,
    operationName = 'google-ai-general'
  ): Promise<T> {
  ...
  static async callAIWithFallback<T>(
    ...
    try {
      ...
      try {
        ...
        // 如果都是熔斷錯誤，返回服務不可用
        if (primaryError instanceof AIServiceUnavailableError && 
            fallbackError instanceof AIServiceUnavailableError) {
          throw new AIServiceUnavailableError(
            'AI 服務暫時全部無法使用，系統正在恢復中',
            'all-services',
            'OPEN'
          );
        }
...
export function withTimeout<T>(
  ...
  return async () => {
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => {
        reject(new AIServiceTimeoutError(
          `AI 服務調用超時 (${timeoutMs}ms)`,
          service,
          timeoutMs
        ));
      }, timeoutMs);
